...
Below is a rather large XML layout with a bunch of different components,
...
In the above example, you'll see that many components have defined an
...
Now we'll need to add the invSubmitButtonClick method to the
AbstractAppState that called the parseLayout method, like so:
...
The quickest way of getting the definition of the event method you are
...
method. Then you simply rename it.
